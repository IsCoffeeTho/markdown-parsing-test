	...
	function updateOutput() {
		function traverseTree(node) {
			...
			if (nodeType == "section") {
				...
				return `<${headerElem} id="${node.text.replace(/^#+/g, "").trim().replace(/([a-zA-Z0-9])|(.)/g, (m, l, p) => l?.toLowerCase() ?? '-')}">${thisOutput}</${headerElem}>`;
				...
				if (node.child(0).child(0).grammarType == "list_marker_dot")
					return `<ol>${thisOutput}</ol>`;
				else
					return `<ul>${thisOutput}</ul>`;
